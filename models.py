...
class my_net(object):
	...
	def build_model(self):
		...
		with tf.name_scope("Model") as scope:
			...
			conv1 = Conv_2D(self.haze_in, output_chan=3, kernel=[5,5], stride=[1,1], padding="SAME", name="Conv1")
			conv2 = Conv_2D(conv1, output_chan=3, kernel=[3,3], stride=[1,1], padding="SAME", name="Conv2")
			pool1 = max_pool(conv2, name="pool1")
			unpool1 = max_unpool(pool1, name="unpool1")
			concat_1 = tf.concat([conv1, unpool1], axis=3, name="Concat_1")
			self.transMap = Conv_2D(concat_1, output_chan=1, kernel=[3,3], stride=[1,1], padding="SAME", name="TMap")
			conv4 = Conv_2D(self.haze_in, output_chan=3, kernel=[3,3], stride=[1,1], padding="SAME", name="Conv4")
			concat_2 = tf.concat([conv4, self.transMap], axis=3, name="Concat_2")
			conv5 = Conv_2D(concat_2, output_chan=3, kernel=[7,7], stride=[1,1], padding="SAME", name="IandT")
			concat_3 = tf.concat([self.haze_in, conv5], axis=3, name="Concat_3")
			self.clearImage = Conv_2D(concat_3, output_chan=3, kernel=[3,3], stride=[1,1], padding="SAME", name="clearImage")
			clear_image_summ = tf.summary.image("Out_Clear", self.clearImage)

		with tf.name_scope("Loss") as scope:
			clearloss = tf.losses.mean_squared_error(self.clear_in, self.clearImage)
			maploss = tf.losses.mean_squared_error(self.trans_in, self.transMap)
			self.loss = clearloss + maploss
		...
		with tf.name_scope("Training") as scope:
			for epoch in range(epoch_size):
				for itr in xrange(0, train_imgs[0].shape[0]-batch_size, batch_size):
					...
					sess_out = self.sess.run(sess_in, {self.haze_in:haze_in, self.clear_in: clear_in, self.trans_in: trans_in, self.train_phase:True})
	...
	def test(self, batch_size):
		...
		y = graph.get_tensor_by_name("Model/clearImage/Relu:0")
		...
		for image in img_name:
			...
			out = sess.run(y, {x:in_img/255.0, is_train:False})
			# plt.imshow(out[2][0])
			# plt.show()
			# plt.imshow(out[3][0])
			# plt.show()
			...
			cv2.imwrite(image[:-4]+"_clear.jpg", out[0]*255.0)
...
class pix2pix(object):
	...
	def _generator(self, input_img):
		with tf.variable_scope("generator") as scope:
			with tf.variable_scope("encoder") as scope:
				...
				enc2 = conv_2d(enc1, output_chan=128, kernel=[4,4], stride=[2,2], use_bn=True, activation=leaky_relu,
					train_phase=self.train_phase, name="enc2")
				print "2",enc2.get_shape()
				enc3 = conv_2d(enc2, output_chan=256, kernel=[4,4], stride=[2,2], use_bn=True, activation=leaky_relu,
					train_phase=self.train_phase, name="enc3")
				print "3",enc3.get_shape()
				enc4 = conv_2d(enc3, output_chan=512, kernel=[4,4], stride=[2,2], use_bn=True, activation=None,
					train_phase=self.train_phase, name="enc4")
				print "4",enc4.get_shape()
				# enc5 = conv_2d(enc4, output_chan=512, kernel=[4,4], stride=[2,2], use_bn=True, activation=leaky_relu,
				# 	train_phase=self.train_phase, name="enc5")
				# print "5",enc5.get_shape()
				# enc6 = conv_2d(enc5, output_chan=512, kernel=[4,4], stride=[2,2], use_bn=True, activation=leaky_relu,
				# 	train_phase=self.train_phase, name="enc6")
				# print "6",enc6.get_shape()
			
			with tf.variable_scope("decoder") as scope:
				# dec6 = deconv_2d(enc6, output_chan=512, kernel=[4,4], stride=[2,2], use_bn=True,
				# 	train_phase=self.train_phase, name="dec6")
				# dec5 = deconv_2d(tf.concat([dec6, enc5], 3), output_chan=512, kernel=[4,4], stride=[2,2], use_bn=True,
				# 	train_phase=self.train_phase, name="dec5")
				# dec4 = deconv_2d(tf.concat([dec5, enc4], 3), output_chan=256, kernel=[4,4], stride=[2,2], use_bn=True,
				# 	train_phase=self.train_phase, name="dec4")
				dec4 = deconv_2d(enc4, output_chan=256, kernel=[4,4], stride=[2,2], use_bn=True,
					train_phase=self.train_phase, name="dec4")
				dec3 = deconv_2d(tf.concat([dec4, enc3], 3), output_chan=128, kernel=[4,4], stride=[2,2], use_bn=True,
	...
	def _discriminator(self, gen_input, gen_output, reuse):
		with tf.variable_scope("discriminator", reuse=reuse) as scope:
			conv1 = conv_2d(tf.concat([gen_input, gen_output],3), output_chan=64, kernel=[4,4], stride=[2,2], use_bn=False, activation=leaky_relu, 
				train_phase=self.train_phase, name="conv1")
			conv2 = conv_2d(conv1, output_chan=128, kernel=[4,4], stride=[2,2], use_bn=True, activation=leaky_relu, 
				train_phase=self.train_phase, name="conv2")
			conv3 = conv_2d(conv2, output_chan=256, kernel=[4,4], stride=[2,2], use_bn=True, activation=leaky_relu, 
				train_phase=self.train_phase, name="conv3")
			conv4 = conv_2d(conv3, output_chan=512, kernel=[4,4], stride=[2,2], use_bn=True, activation=leaky_relu, 
				train_phase=self.train_phase, name="conv4")
			conv5 = conv_2d(conv4, output_chan=1, kernel=[4,4], stride=[1,1], use_bn=False, activation=tf.sigmoid, 
	...
	def test(self, input_imgs, batch_size):
		...
		saver = tf.train.import_meta_graph(self.save_path+'pix2pix-120.meta')
		print self.save_path
		saver.restore(sess,tf.train.latest_checkpoint(self.save_path))
		print self.save_path
		graph = tf.get_default_graph()
		...
		for itr in xrange(0, input_imgs.shape[0], batch_size):
			...
			input_img = input_imgs[itr:end]
			out = sess.run(y, {x:input_img/127.5-1, is_train:False})
			if itr==0:
				tot_clr = out
			else:
				tot_clr = np.concatenate((tot_clr, out))
